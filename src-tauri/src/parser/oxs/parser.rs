...
use crate::{error::Result, pattern::Pattern};
...
pub fn parse_pattern(path: impl AsRef<std::path::Path>) -> Result<Pattern> {
  ...
  let mut reader = quick_xml::Reader::from_file(path.as_ref()).unwrap();
  let mut buf = Vec::new();
    ...
    match reader.read_event_into(&mut buf) {
      Ok(Event::Empty(ref e)) => {
        if e.name().as_ref() == b"properties" {
          ...
          let oxs_version: OxsVersion = attributes.get("oxsversion").unwrap().parse().unwrap();
          let software: Software = attributes.get("software").unwrap().parse().unwrap();
